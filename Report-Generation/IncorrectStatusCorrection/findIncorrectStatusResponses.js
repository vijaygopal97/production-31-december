...
const OUTPUT_DIR = path.join(__dirname, '../../Report-Generation/IncorrectStatusCorrection');
 ...
 */
function checkResponseCompleteness(responseDoc) {
  const issues = [];
  ...
  // 2. Check Gender question: "Please note the respondent's gender"
  ...
  const genderResponse = findResponseByPatterns(responses, genderPatterns);
  ...
  if (isCAPI) {
    ...
    const acResponse = findResponseByPatterns(responses, acPatterns);
...
async function findIncorrectStatusResponses() {
  try {
    ...
    for await (const responseDoc of cursor) {
      ...
      const checkResult = checkResponseCompleteness(responseDoc);
