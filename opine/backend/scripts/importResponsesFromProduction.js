/**
 * Import Survey Responses from Production to Development Database
 * 
 ...
 * 2. Fetches all survey responses (or for a specific survey)
 * 3. Connects to development database
 * 4. Imports responses, handling duplicates and preserving relationships
 * 
 * SAFETY FEATURES:
...
const importResponses = async () => {
  ...
  try {
    ...
    const ProdSurvey = prodConnection.model('Survey', Survey.schema);
    ...
    const DevSurvey = devConnection.model('Survey', Survey.schema);
    
    console.log('✅ Connected to DEVELOPMENT database\n');

    // Step 3: Build query for production responses (READ-ONLY)
    const query = {};
    if (TARGET_SURVEY_ID) {
      query.survey = new mongoose.Types.ObjectId(TARGET_SURVEY_ID);
      console.log(`📋 Step 3: Fetching responses for survey: ${TARGET_SURVEY_ID} (READ-ONLY)`);
    } else {
      console.log('📋 Step 3: Fetching ALL survey responses (READ-ONLY)');
    }

    // Step 4: Count responses in production (READ-ONLY operation)
    console.log('   🔍 Counting responses in production (READ-ONLY)...');
    const totalCount = await ProdSurveyResponse.countDocuments(query);
    console.log(`📊 Total responses in production: ${totalCount} (no data modified)\n`);

    if (totalCount === 0) {
      console.log('⚠️  No responses found in production. Exiting.');
      return;
    }

    // Step 5: Fetch responses in batches
    ...
    console.log(`📦 Step 4: Importing responses in batches of ${BATCH_SIZE}...\n`);
    ...
    for (let skip = 0; skip < totalCount; skip += BATCH_SIZE) {
      ...
      const prodResponses = await ProdSurveyResponse.find(query)
        .populate('survey', '_id')
      ...
      for (const prodResponse of prodResponses) {
        try {
          // Check if response already exists in dev (by responseId or _id)
          const existingResponse = await DevSurveyResponse.findOne({
            $or: [
              { responseId: prodResponse.responseId },
              { _id: prodResponse._id },
              { sessionId: prodResponse.sessionId }
            ]
          }).lean();
          ...
          const surveyId = prodResponse.survey?._id || prodResponse.survey;
          const devSurvey = await DevSurvey.findOne({ _id: surveyId }).lean();
          
          if (!devSurvey) {
            console.log(`   ⚠️  Survey ${surveyId} not found in dev, skipping response ${prodResponse.responseId || prodResponse._id}`);
            skippedCount++;
            continue;
          }

          // Check if interviewer exists in dev (by memberId or _id)
          const interviewerId = prodResponse.interviewer?._id || prodResponse.interviewer;
          ...
          if (interviewerId) {
            ...
            if (!devInterviewer && prodResponse.interviewer?.memberId) {
              devInterviewer = await DevUser.findOne({ memberId: prodResponse.interviewer.memberId }).lean();
            }
          }

          // Prepare response data for import
          const responseData = {
            ...prodResponse,
            survey: surveyId,
            interviewer: devInterviewer?._id || interviewerId, // Use dev interviewer ID if found, otherwise original
            _id: prodResponse._id, // Preserve original _id
            createdAt: prodResponse.createdAt,
            updatedAt: prodResponse.updatedAt
          };

          // Remove populated fields that shouldn't be in the document (local copy only, not affecting production)
          delete responseData.__v; // Remove version key if present (local copy only)

          // Clean up empty string enum values in verificationData (convert '' to undefined/null)
          if (responseData.verificationData) {
            // Clean criteria object
            if (responseData.verificationData.criteria) {
              ...
              criteriaFields.forEach(field => {
                if (responseData.verificationData.criteria[field] === '') {
                  delete responseData.verificationData.criteria[field];
                }
              });
            }
            ...
            verificationFields.forEach(field => {
              if (responseData.verificationData[field] === '') {
                delete responseData.verificationData[field];
              }
            });
          }

          // Import response (WRITE to dev only)
          // Use runValidators: false to skip validation for edge cases (empty strings in enums)
          await DevSurveyResponse.create([responseData], { runValidators: false });
          importedCount++;

        } catch (error) {
          errorCount++;
          const errorInfo = {
            responseId: prodResponse.responseId || prodResponse._id,
            error: error.message
          };
          errors.push(errorInfo);
          console.error(`   ❌ Error importing response ${errorInfo.responseId}:`, error.message);
        }
      }
    ...
    console.log('📊 Summary:');
    console.log('================================================================================');
    console.log(`   Total responses in production: ${totalCount}`);
    ...
    const devCount = await DevSurveyResponse.countDocuments(TARGET_SURVEY_ID ? { survey: TARGET_SURVEY_ID } : {});
    console.log(`   Responses in development database: ${devCount}`);
    console.log();
...
module.exports = { importResponses };
