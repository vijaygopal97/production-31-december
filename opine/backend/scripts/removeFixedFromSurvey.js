...
async function removeFixedFromSurvey() {
  try {
    // Connect to MongoDB
      ...
      if (section.questions && Array.isArray(section.questions)) {
        ...
        for (let questionIndex = 0; questionIndex < section.questions.length; questionIndex++) {
          ...
          if (question.isFixed || question.isLocked) {
            console.log(`🔍 Found fixed/locked question: ${question.text}`);
            
            // Remove isFixed and isLocked
            const updatedQuestion = { ...question };
            delete updatedQuestion.isFixed;
            delete updatedQuestion.isLocked;
            
            updatedSections[sectionIndex].questions[questionIndex] = updatedQuestion;
            updated = true;
            console.log(`✅ Removed isFixed/isLocked from question`);
          }
        }
      }
    }
